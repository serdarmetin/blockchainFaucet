// DRF employing AMF
// This is the initial version of multi-round ADRF, 
...
//	Update epoch & round: epoch starts from 1 to prevent triggering "already d" in the first epoch
	if(epoch < (block.number - offset) / es + 1){
		...
		unitShare = (c[selector][0] * p * p * p * p) / (mds[selector] * crd[selector][0]);
		for(uint i = 0; i < nore; i++){
			if(unitShare > (c[selector][i] * p * p * p * p) / (mds[selector] * crd[selector][i]))
				unitShare = (c[selector][i] * p * p * p * p) / (mds[selector] * crd[selector][i]);
		}
...
//	Epoch is up-to-date, so update only the round: round also starts from 1 to prevent triggering "already claimed" in the first round
		...
		uint8 selector = uint8((epoch) % 2);
		if(nod[selector] > 0){
			unitShare = (lc[selector][0] * p * p * p * p) / (mds[selector] * crd[selector][0]);
			for(uint i = 0; i < nore; i++){
				if(unitShare > (lc[selector][i] * p * p * p * p) / (mds[selector] * crd[selector][i]))
					unitShare = (lc[selector][i] * p * p * p * p) / (mds[selector] * crd[selector][i]);
			}
		}
	}
	}	
