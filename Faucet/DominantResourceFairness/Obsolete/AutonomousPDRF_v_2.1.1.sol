// DRF employing AMF
// This is the initial version of single round ADRF, 
...
//	Update epoch & round: epoch starts from 1 to prevent triggering "already d" in the first epoch
		...
		unitShare = (c[selector][0] * p * p) / (mds[selector] * crd[selector][0]);
		for(uint i = 0; i < nore; i++){
			if(unitShare > (c[selector][i] * p * p) / (mds[selector] * crd[selector][i]))
				unitShare = c[selector][i] * p * p / (mds[selector] * crd[selector][i]);
		}
