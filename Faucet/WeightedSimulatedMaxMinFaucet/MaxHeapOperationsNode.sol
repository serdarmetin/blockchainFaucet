//  Weight is key and volume is value
...
contract Heaped{
    ...
    function push(node[] memory _heap, uint _length, node memory _node) pure public {
        ...
        while(i != 0){
            if(_heap[i].weight > _heap[j].weight){
    ...
    function pop(node[] memory _heap, uint _length) pure public {
        ...
        while(left < leaf){
            if(right < leaf){
                if(_heap[right].weight > _heap[left].weight){
                   if(_heap[top].weight < _heap[right].weight){
            ...
            if(_heap[top].weight < _heap[left].weight){
